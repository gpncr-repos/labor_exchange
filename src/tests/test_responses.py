...
async def test_get_recponces_by_job_id_and_user_id(sa_session):
    ...
    current_responses = await responses_query.get_response_by_job_id_and_user_id(sa_session,job_id=job.id,user_id=user.id)
...
async def test_create(sa_session):
    ...
    res = await responses_query.response_create(sa_session,response_schema=response,user_id=worker.id)
...
async def test_update(sa_session):
    ...
    current_responses = await responses_query.get_response_by_job_id_and_user_id(sa_session,job_id=job.id,user_id=worker.id)
