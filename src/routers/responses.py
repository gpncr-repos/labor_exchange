...
async def read_all_my_responses(
    ...
    if current_user.is_company is True:
        return await responses_queries.get_response_by_user_id(db=db, user_id=current_user.id, choice=0)
    return await responses_queries.get_response_by_user_id(db=db, user_id=current_user.id, choice=1)
